...
export interface ApprovalItem {
  ...
  isVoided?: boolean; // To mark if a request is voided
  voidReason?: string; // Reason for voiding
}

const ITEMS_PER_PAGE = 10;

export const mockAllRequestsData: ApprovalItem[] = [
  {
  ...
  // Additional PO examples for testing
  {
...
export const renderRequestPayloadDetailsDialog = (payload: RequestPayload, requestType: RequestType) => {
...
export const renderWorkflowProgress = (request: ApprovalItem) => {
...
export default function AllRequestsPage() {
  ...
  const distinctStatuses = React.useMemo(() => {
    const statuses = new Set(requests.map(req => req.currentStepName));
     // Add "Voided" if not already present from data
