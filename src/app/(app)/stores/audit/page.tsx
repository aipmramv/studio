...
import { PackageSearch, Search, Filter as FilterIcon, ChevronsLeft, ChevronsRight, Package as PackageIcon, Tag as TagIcon, Warehouse as WarehouseIcon, AlertTriangle, CheckCircle, Circle } from "lucide-react";
...
const getStatusBadgeVariant = (status: AvailabilityStatus) => {
  ...
  if (status === "Low Stock") return "secondary"; // Or 'outline' if you prefer for warning
  return "default"; // For 'In Stock'
};

const getStatusIcon = (status: AvailabilityStatus) => {
  if (status === "Out of Stock") return <X className="w-4 h-4 mr-2 text-destructive" />;
  if (status === "Low Stock") return <AlertTriangle className="w-4 h-4 mr-2 text-yellow-500" />; // Using a distinct color for low stock
  return <CheckCircle className="w-4 h-4 mr-2 text-green-500" />; // Using a distinct color for in stock
};
