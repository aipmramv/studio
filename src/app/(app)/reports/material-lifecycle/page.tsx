        ...
        <CardHeader>
          <CardTitle className="font-headline flex items-center"><PackageSearch className="w-5 h-5 mr-2 text-primary"/>Report Filters & Actions</CardTitle>
            <div className="flex flex-wrap items-center gap-2 pt-4">
              ...
              <Select value={filterRequestType || "all"} onValueChange={(val) => setFilterRequestType(val === 'all' ? '' : val as RequestType | "")}><SelectTrigger className="w-full sm:w-[160px]"><SelectValue placeholder="Request Type" /></SelectTrigger><SelectContent><SelectItem value="all">All Request Types</SelectItem>{REQUEST_TYPES.map(type => <SelectItem key={type} value={type}>{type}</SelectItem>)}</SelectContent></Select>
              <Select value={filterLifecycleStage || "all"} onValueChange={(val) => setFilterLifecycleStage(val === 'all' ? '' : val as LifecycleStage | "")}><SelectTrigger className="w-full sm:w-[160px]"><SelectValue placeholder="Lifecycle Stage" /></SelectTrigger><SelectContent><SelectItem value="all">All Stages</SelectItem>{LIFECYCLE_STAGES.map(stage => <SelectItem key={stage} value={stage}>{stage}</SelectItem>)}</SelectContent></Select>
              <Select value={filterDepartment || "all"} onValueChange={(val) => setFilterDepartment(val === 'all' ? '' : val as Department | "")}><SelectTrigger className="w-full sm:w-[160px]"><SelectValue placeholder="Possessed By" /></SelectTrigger><SelectContent><SelectItem value="all">All Departments</SelectItem>{DEPARTMENTS.map(dept => <SelectItem key={dept} value={dept}>{dept}</SelectItem>)}</SelectContent></Select>
              <Select value={filterRequestedBy || "all"} onValueChange={(val) => setFilterRequestedBy(val === 'all' ? '' : val as string)}><SelectTrigger className="w-full sm:w-[160px]"><SelectValue placeholder="Requested By" /></SelectTrigger><SelectContent><SelectItem value="all">All Users</SelectItem>{distinctRequesters.map(user => <SelectItem key={user} value={user}>{user}</SelectItem>)}</SelectContent></Select>
