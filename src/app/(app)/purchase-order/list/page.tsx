...
export default function PurchaseOrderListPage() {
  ...
  const [editingRequest, setEditingRequest] = React.useState<PurchaseOrderDisplayItem | null>(null);
  ...
  const filteredPurchaseOrders = React.useMemo(() => {
    ...
    if (filterStatus) {
      if (filterStatus === "Voided") {
         tempPOs = tempPOs.filter(po => po.isVoided || po.currentStepName === "Request Voided");
      } else {
    ...
    if (searchTerm) {
      tempPOs = tempPOs.filter(po =>
        ...
        po.payload.vendorName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleEditRequest = (item: PurchaseOrderDisplayItem) => {
    setEditingRequest(item);
    setIsEditDialogOpen(true);
  };
  
  const handleSaveEdit = (updatedData: PurchaseOrderFormData) => {
    if (!editingRequest) return;
    const updatedItem: PurchaseOrderDisplayItem = { ...editingRequest, payload: updatedData };
    ...
    setPurchaseOrders(prev => prev.map(item => item.id === editingRequest.id ? updatedItem : item));
  ...
  const handleConfirmVoid = () => {
    ...
    const voidedItem: PurchaseOrderDisplayItem = {
      ...requestToVoid,
      isVoided: true,
    ...
    setPurchaseOrders(prevItems => prevItems.map(item => item.id === requestToVoid.id ? voidedItem : item));
  ...
  const handleAddCommentForDetail = () => {
    ...
    const updatedRequest = { ...selectedRequestDetail, history: [ ...selectedRequestDetail.history, { stepId: selectedRequestDetail.currentStepId, stepName: `Comment on: ${selectedRequestDetail.currentStepName}`, actor: user.displayName || "Current User", action: "commented" as const, timestamp: new Date().toISOString(), comment: newComment, }, ], };
  ...
  const handleUpdateSapNumber = () => {
    ...
    const updatedRequest: PurchaseOrderDisplayItem = {
      ...
      history: [ ...selectedRequestDetail.history, { stepId: selectedRequestDetail.currentStepId, stepName: "SAP PO Number Updated", action: "approve", actor: user.displayName || "System", timestamp: new Date().toISOString(), comment: `SAP PO Number updated to: ${sapNumberInput}`, }, ],
    };
    ...
    <div className="space-y-8">
            ...
            <Popover>
              ...
              <PopoverContent className="w-80 p-4 space-y-4" align="end">
                <div>
                  ...
                  <Calendar id="date-range-po" mode="range" selected={dateRange} onSelect={setDateRange} className="rounded-md border p-0 mt-1" numberOfMonths={1} />
                </div>
                <div>
        ...
        <CardContent>
          <div className="flex items-center gap-4 mb-6">
            <div className="relative flex-grow">
              ...
              <Input placeholder="Search by PO ID, Vendor, Requester, Status..." className="pl-10" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} />
            </div>
          </div>
            ...
            <>
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableCaption>Showing {paginatedPOs.length} of {filteredPurchaseOrders.length} Purchase Orders. Page {currentPage} of {totalPages}.</TableCaption>
                </Table>
              </div>
          ...
          ) : (<p className="text-center text-muted-foreground py-8">No Purchase Orders match your current search/filters, or no POs available.</p>)}
        </CardContent>
      </Card>
      {selectedRequestDetail && (
        <Dialog open={isDetailDialogOpen} onOpenChange={(isOpen) => { setIsDetailDialogOpen(isOpen); if (!isOpen) { setSelectedRequestDetail(null); setNewComment(""); } }}>
          <DialogContent className="sm:max-w-3xl">
            <DialogHeader><DialogTitle className="flex items-center">{getRequestTypeIcon(selectedRequestDetail.requestType, "w-6 h-6 mr-2 text-primary")}Purchase Order Details: {selectedRequestDetail.id}</DialogTitle><DialogDescription>Detailed information, workflow progress, and approval timeline for PO {selectedRequestDetail.id}.</DialogDescription></DialogHeader>
                    ...
                    <CardContent className="space-y-1 text-sm">
                      ...
                      {selectedRequestDetail.payload.sapOrderNumber ? (
                        <p><strong>SAP PO Number:</strong> <span className='font-bold text-primary'>{selectedRequestDetail.payload.sapOrderNumber}</span></p>
                          ...
                          selectedRequestDetail.currentStepId === 'po_sap_creation' && (user?.role === 'admin' || user?.role === 'finance_team') && (
                          <div className="mt-4 p-4 border-t bg-amber-50 rounded-md">
                            ...
                            <p className="text-xs text-muted-foreground mb-2">Enter the SAP PO number to proceed to the next step.</p>
                            <div className="flex gap-2 mt-2">
                                <Input value={sapNumberInput} onChange={(e) => setSapNumberInput(e.target.value)} placeholder="Enter SAP PO Number"/>
      ...
      {editingRequest && isEditDialogOpen && (
        <Dialog open={isEditDialogOpen} onOpenChange={(isOpen) => { if (!isOpen) { setEditingRequest(null); setIsEditDialogOpen(false); } }}>
          <DialogContent className="sm:max-w-3xl">
            <DialogHeader>
              <DialogTitle className="flex items-center"><Edit className="w-5 h-5 mr-2 text-primary" /> Edit Purchase Order: {editingRequest.id}</DialogTitle>
              <DialogDescription>Modify the details of this Purchase Order request.</DialogDescription>
            </DialogHeader>
            <PurchaseOrderForm initialData={editingRequest.payload} isEditing={true} onSave={handleSaveEdit} onCancel={() => { setIsEditDialogOpen(false); setEditingRequest(null); }} />
          </DialogContent>
        </Dialog>
      )}
        ...
        <AlertDialog open={isVoidDialogOpen} onOpenChange={setIsVoidDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>Are you sure you want to void Purchase Order <strong>{requestToVoid.id}</strong>? This action cannot be undone.</AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-2 my-4"><label htmlFor="voidReason" className="text-sm font-medium">Reason for Voiding (Optional)</label><Textarea id="voidReason" placeholder="Enter reason..." value={voidReason} onChange={(e) => setVoidReason(e.target.value)}/></div>
