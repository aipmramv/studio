...
const MOCK_CONDITIONS_INIT = ["Working", "Needs Repair", "Down", "Good", "Fair", "Poor"];

export default function ConditionMasterPage() {
  const [conditions, setConditions] = React.useState([...MOCK_CONDITIONS_INIT]);
  ...
  const handleOpenDialog = (index: number | null = null) => {
    if (index !== null) {
      ...
      setCurrentValue(conditions[index]);
    } else {
  ...
  const handleSave = () => {
    if (!currentValue.trim()) {
      toast({ title: "Validation Error", description: "Condition name cannot be empty.", variant: "destructive" });
      return;
    }

    if (editingIndex !== null) {
      const updatedData = [...conditions];
      updatedData[editingIndex] = currentValue;
      setConditions(updatedData);
      toast({ title: "Success", description: "Condition updated successfully." });
    } else {
      setConditions([...conditions, currentValue]);
      toast({ title: "Success", description: "New condition added successfully." });
    }
  ...
  const handleDelete = () => {
    if (deletingIndex !== null) {
      const updatedData = conditions.filter((_, i) => i !== deletingIndex);
      setConditions(updatedData);
      ...
      <Card>
        ...
        <CardContent>
           <Table>
              ...
              <TableCaption>{conditions.length} condition(s) found.</TableCaption>
            </Table>
        </CardContent>
      </Card>
       ...
       <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{editingIndex !== null ? 'Edit Condition' : 'Add New Condition'}</DialogTitle>
                <DialogDescription>
                    {editingIndex !== null ? 'Update the name of the condition.' : 'Enter the name for the new condition.'}
                </DialogDescription>
            </DialogHeader>
