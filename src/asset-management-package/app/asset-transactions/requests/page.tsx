...
const mockRequests: OwnershipRequest[] = [
    { id: "REQ-001", assets: [{ assetId: "ASSET-001", assetNumber: "KONE-RD-LAP-001", currentDepartment: "R&D" }], toDepartment: "Maintenance", reason: "Required for new diagnostics bench.", requestedBy: "Praveen S.", requestedOn: new Date(), status: "Pending" },
    { id: "REQ-002", assets: [{ assetId: "ASSET-003", assetNumber: "KONE-MAINT-TOOL-001", currentDepartment: "Maintenance" }], toDepartment: "Production", reason: "Temporary loan for line setup.", requestedBy: "Ram Kumar", requestedOn: new Date(new Date().setDate(new Date().getDate() - 2)), status: "Approved" },
];
...
export default function AssetRequestPage() {
  ...
  const { user } = useAuth();
  const allAssets = mockAssetData; // Using mock data
  ...
  const handleRequestSubmit = () => {
    if (selectedAssets.length === 0 || !toDept || !reason || !user) {
  ...
  const handleRequestAction = (requestId: string, action: "Approve" | "Reject") => {
    setRequests(prev => prev.map(req => req.id === requestId ? { ...req, status: action === 'Approve' ? 'Approved' : 'Rejected' } : req));
