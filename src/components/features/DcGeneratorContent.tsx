...
const mockDCData: DCData = {
  dcNumber: "DC20240730001",
...
export function DcGeneratorContent() {
  const [dcData, setDcData] = React.useState<DCData>(mockDCData);
  const [eWayBillRef, setEWayBillRef] = React.useState(dcData.eWayBillLink || "");
  ...
  const handleSubmitDc = () => {
    ...
    console.log("Submitting DC:", { ...dcData, eWayBillLink: eWayBillRef });
    // Simulate API call
    setTimeout(() => {
      toast({
        title: "DC Submitted Successfully",
        description: `DC ${dcData.dcNumber} has been finalized.`,
      });
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            ...
            <CardDescription>Review and finalize the auto-generated Delivery Challan.</CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        {!canSubmit && (
          <div className="p-4 mb-4 text-sm rounded-md bg-destructive/10 text-destructive border border-destructive flex items-center">
            ...
            The deadline for DC submission (3:30 PM) has passed for today.
          </div>
        )}
        <div className="grid grid-cols-1 gap-4 p-4 border rounded-lg md:grid-cols-3 bg-card">
          <div><strong>DC Number:</strong> {dcData.dcNumber}</div>
          <div><strong>Date:</strong> {dcData.date}</div>
          <div><strong>Vehicle No:</strong> {dcData.vehicleNumber}</div>
          <div><strong>Source:</strong> {dcData.source}</div>
          <div><strong>Destination:</strong> {dcData.destination}</div>
          <div><strong>Approved Request ID:</strong> {dcData.approvedRequestID}</div>
        </div>
